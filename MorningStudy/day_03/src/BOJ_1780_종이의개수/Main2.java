...
public class Main2 {
	...
	public static void cutPaper(int[][] arr, int r, int c, int N) {
		if (checkIsSame(r, c, N)) { // 숫자가 같다면 어떤 종이의 숫자를 올릴지 선택
			...
			cutPaper(arr, r, c, side); // 9개로 나눈 종이에 대해 cutPaper() 메서드를 실행
			cutPaper(arr, r, c + side, side);
			cutPaper(arr, r, c + 2*side, side);
			cutPaper(arr, r + side, c, side);
			cutPaper(arr, r + side, c + side, side);
			cutPaper(arr, r + side, c + 2*side, side);
			cutPaper(arr, r + 2*side, c, side);
			cutPaper(arr, r + 2*side, c + side, side);
			cutPaper(arr, r + 2*side, c + 2*side, side);
		}
	}
