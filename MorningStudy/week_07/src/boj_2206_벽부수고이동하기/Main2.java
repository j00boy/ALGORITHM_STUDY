...
public class Main2 {
	...
	public static void main(String[] args) {
		...
		map = new int[N][M];
		visited = new boolean[N][M];		// 벽 안 부셨을 떄
		n_visited = new boolean[N][M];		// 벽을 1번이라도 부셨을 때 방문체크
	...
	public static int BFS(int[] arr) {
		...
		while(!queue.isEmpty()) {
			...
			for(int d = 0; d < 4; d++) {
				...
				} else if(check(nr, nc) && map[nr][nc] == 0 && now_breakWall == 1 && !n_visited[nr][nc]) {	// 벽을 한 번 부셨으면, 이 조건만 이제 유효함
					if(nr == N-1 && nc == M-1) {
						return now_count+1;
					}
				...
				// 부신 적이 없는 상태에서 벽을 만나면, 부순다. + 부순 기점에서 n_visited에 체크
				} else if(check(nr, nc) && map[nr][nc] == 1 && now_breakWall == 0) {		// 한 번도 안 부순 상태에서 벽을 만나면? 부순다.
					if(nr == N-1 && nc == M-1) {
						return now_count+1;
					}
					queue.add(new int[] {nr, nc, 1, now_count + 1});
